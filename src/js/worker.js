...
const checkTime = (t) => {
    ...
    self.postMessage(//worker posts message "move" with current t to the main thread
        {
            action: "move",
            time: t
        }
    ...
    setTimeout(() => { checkTime(--t) }, 1000); //start checkTime over 1 second
}

self.addEventListener('message', function (e) { //add event listener to the events that come from the main thread
